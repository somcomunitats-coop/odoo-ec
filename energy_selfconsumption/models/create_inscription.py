...
class CreateInscription(models.AbstractModel):
    ...
    def _determine_tariff(self, contracted_power, values):
        ...
        return values.get("tariff") or ("2.0TD" if contracted_power <= 15 else ("3.0TD" if contracted_power <= 300 else "6.1TD"))

    def _create_supply_point(self, values, project, partner, owner, tariff, used_in_selfconsumption, create_bank):
        """Crea el punto de suministro si no existe ya."""
        supply_point = self.env["energy_selfconsumption.supply_point"].sudo().search([("code", "=", values["supplypoint_cups"])])
        #if supply_point:
        ...
        mandate = self._create_bank_mandate(values, partner, project) if create_bank else None
        effective_date = self._get_effective_date(values)
        annual_electricity_use = values.get("inscriptionselfconsumption_annual_electricity_use", False)

        self._create_inscription_record(values, project, partner, effective_date, mandate, participation, annual_electricity_use, supply_point)
        
        return False, _("You have successfully registered.")

    def create_inscription(self, values, project, create_bank=True, supplypoint_owner_id_same=True, conf_vulnerability_situation=False, conf_used_in_selfconsumption=False):
        ...
        if not partner:
            return True, _("Partner with VAT:<b>{vat}</b> was not found.").format(vat=values["inscription_partner_id_vat"])

        if not self._is_cooperator(partner, project):
            return True, _("Partner with VAT:<b>{vat}</b> is not a cooperator.").format(vat=values["inscription_partner_id_vat"])

        values.setdefault("date_format", "%Y-%m-%d")

        if self._is_partner_already_registered(project, partner):
            return True, _("Partner with VAT {vat} is already registered in project {code}").format(vat=partner.vat, code=project.code)

        owner = self._get_owner(values, project, partner, supplypoint_owner_id_same, conf_vulnerability_situation)
        if not owner:
        ...
        used_in_selfconsumption = "yes" if conf_used_in_selfconsumption and values.get("supplypoint_used_in_selfconsumption") == "yes" else "no"

        return self._create_supply_point(values, project, partner, owner, tariff, used_in_selfconsumption, create_bank)
    ...
    def _is_cooperator(self, partner, project):
        ...
        return bool(self.env["cooperative.membership"].sudo().search([
            ("company_id", '=', project.company_id.id),
    ...
    def _create_bank_mandate(self, values, partner, project):
        ...
        return self.env["account.banking.mandate"].with_company(project.company_id).sudo().create({
    ...
    def _get_mandate_auth_date(self, values):
        ...
        mandate_auth_date = values.get("mandate_auth_date", datetime.now().strftime(date_format))
    ...
    def _is_partner_already_registered(self, project, partner):
        ...
        return project.inscription_ids.filtered_domain([("partner_id", "=", partner.id)])
    ...
    def _get_participation(self, values, project):
        ...
        if "inscriptionselfconsumption_participation" in values:
            domain.append(("quantity", "=", float(values["inscriptionselfconsumption_participation"])))
    ...
    def _get_effective_date(self, values):
        ...
        date_format = values.get("date_format", "%Y-%m-%d")
        effective_date = values.get("effective_date", datetime.now().strftime(date_format))
        return datetime.strptime(effective_date, date_format).date()

    def _create_inscription_record(self, values, project, partner, effective_date, mandate, participation, annual_electricity_use, supply_point):
    ...
    def _get_owner(self, values, project, partner, supplypoint_owner_id_same, conf_vulnerability_situation):
        ...
        vulnerability_situation = values.get("supplypoint_owner_id_vulnerability_situation", "no") if conf_vulnerability_situation else "no"
        ...
        if not owner:
            return self._create_new_owner(values, project, country, state, lang, vulnerability_situation, formatted_birthdate)
    ...
    def _get_country(self, values, project):
        ...
        return self.env["res.country"].sudo().search([("code", "=", values.get("country", project.company_id.partner_id.country_id.code))])

    def _get_state(self, values, country):
        """Obtiene el estado basado en los valores y el país."""
        return self.env["res.country.state"].sudo().search([("code", "=", values.get("state")), ("country_id", "=", country.id)])

    def _get_language(self, values):
        """Obtiene el idioma basado en los valores."""
        lang_code = values.get("supplypoint_owner_id_lang")
        return self.env["res.lang"].sudo().search([("iso_code", "=", lang_code)]) if lang_code else None
    ...
    def _get_existing_contact_owner(self, values):
        ...
        return self.env["res.partner"].sudo().search([("vat", "=", values["supplypoint_owner_id_vat"]),("type", "=", "contact")])
    
    def _get_existing_owner_self_consumption_owner(self, values):
        """Busca un propietario existente basado en el VAT."""
        return self.env["res.partner"].sudo().search([("vat", "=", values["supplypoint_owner_id_vat"]),("type", "=", "owner_self-consumption")])

    def _create_new_owner(self, values, project, country, state, lang, vulnerability_situation, formatted_birthdate):
    ...
    def _update_owner_address(self, project, owner, values, country, state):
        ...
        if exists:
            exists.sudo().write({
                ...
                "lastname": values["supplypoint_owner_id_lastname"],
                "gender": values.get("supplypoint_owner_id_gender"),
                "vulnerability_situation": values.get("supplypoint_owner_id_vulnerability_situation", "no"),
                "birthdate_date": self._get_formatted_birthdate(values),
                "phone": values.get("supplypoint_owner_id_phone"),
                "lang": self._get_language(values).code if self._get_language(values) else None,
                "email": values.get("supplypoint_owner_id_email"),
                "vat": values["supplypoint_owner_id_vat"],
