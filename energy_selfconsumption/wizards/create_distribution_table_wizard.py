...
class CreateDistributionTableWizard(models.TransientModel):
    ...
    percentage_of_distributed_power = fields.Float(string="Percentage of distributed power", readonly=True)
    ...
    distribute_excess = fields.Selection(
        DISTRIBUTE_EXCESS_VALUES, default="no", required=True, string="Distribute excess"
    )

    type_distribute_excess = fields.Selection(
        TYPE_DISTRIBUTE_EXCESS_VALUES, default="proportional", required=True, string="Type distribute excess"
    )

    @api.model
    def default_get(self, default_fields):
        # OVERRIDE
        default_fields = super(CreateDistributionTableWizard, self).default_get(default_fields)
        ...
        selfconsumption = self.env["energy_selfconsumption.selfconsumption"].sudo().search(
            [('project_id','=',inscriptions[0].project_id.id)])
        ...
        default_fields["distributed_power"] = sum(map(lambda inscription: inscription.participation_quantity, inscriptions))

        default_fields["percentage_of_distributed_power"] = (default_fields["distributed_power"] / 
                                                             default_fields["max_distributed_power"]) * 100
    ...
    def create_distribution_table(self):
        ...
        for inscription in inscriptions:
            values_list.append(self.get_supply_point_assignation_values(inscription, distribution_table, len(inscriptions)))
    ...
    def get_supply_point_assignation_values(self, inscription, distribution_table, len_inscriptions):

        coefficient = inscription.participation_quantity

        if self.distribute_excess == "yes":
            
            distribute_excess_float = self.max_distributed_power - self.distributed_power

            if self.type_distribute_excess == "proportional":
                coefficient += distribute_excess_float * (inscription.participation_quantity 
