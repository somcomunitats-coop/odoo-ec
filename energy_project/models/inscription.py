...
class Inscription(models.Model):
    ...
    def unlink(self):
        for res in self:
            matching_assignations = res.has_matching_supply_assignations()
            if len(matching_assignations) > 0:
                ...
                raise ValidationError(
                    _(
                        "The inscription cannot be deleted. It is related to a "
                        "distribution table with state: {table_state}"
                    ).format(table_state=table_states)
                )
            supply_point_assignations = res.get_matching_supply_assignations_to_remove()
