...
from ..config import (  # CANARY_COOP_ACCOUNT_REF_NONPROFIT,; CANARY_RECURRING_FEE_ACCOUNT_REF,; CANARY_RECURRING_FEE_ACCOUNT_REF_NONPROFIT,
...
class ResCompany(models.Model):
    ...
    def get_company_share_recurring_fee_service_account(self):
        if self.legal_form in _LEGAL_FORM_VALUES_NON_PROFIT:
            # if self.is_canary():
            #     return self.env.ref(CANARY_RECURRING_FEE_ACCOUNT_REF_NONPROFIT.format(self.id))
            # else:
            #     return self.env.ref(RECURRING_FEE_ACCOUNT_REF_NONPROFIT.format(self.id))
            return self.env.ref(RECURRING_FEE_ACCOUNT_REF_NONPROFIT.format(self.id))
        # if self.is_canary():
        #     return self.env.ref(CANARY_RECURRING_FEE_ACCOUNT_REF.format(self.id))
        # else:
